...
SKN_PTERO_LOGO
    ...
    printf '%b\n' "  ${CYAN}│${RESET} Version:  ${LIGHT_BLUE}Latest available${RESET}"
    printf '%b\n' "  ${CYAN}└───────────────────────────${RESET}"
...
SKN_APT_CONF
    ...
    # Version selection is intentionally removed from SKN.
    # The upstream installer resolves and installs the latest available
    # Pterodactyl Panel release automatically.
...
SKN_EXPECT
    ...
    if expect "$expect_script"; then
        if [[ -f /var/www/pterodactyl/artisan ]]; then
            ...
            printf '%b\n' "${LIGHT_BLUE}Panel release: Latest available${RESET}"
