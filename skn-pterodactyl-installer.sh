...
theme_menu() {
    while true; do
        clear
        printf '%b\n' "${CYAN}╭──────────────────────────────────────────────────────────╮${RESET}"
        printf '%b\n' "${CYAN}│${RESET}                    ${WHITE}SKN THEME MANAGER${RESET}                     ${CYAN}│${RESET}"
        printf '%b\n' "${CYAN}╰──────────────────────────────────────────────────────────╯${RESET}"
        printf ' ${WHITE}User:${RESET} %s  ${WHITE}Host:${RESET} %s  ${WHITE}Time:${RESET} %s\n' "$(id -un 2>/dev/null || echo root)" "$(hostname 2>/dev/null || echo unknown)" "$(date +%H:%M 2>/dev/null || echo --:--)"
        printf '%b\n' " ${CYAN}──────────────────────────────────────────────────────────${RESET}"
        printf '%b\n\n' " ${LIGHT_BLUE}SELECT A THEME UI:${RESET}"
        printf '   ${WHITE}[1]${RESET} nebula theme             %b    ${WHITE}[2]${RESET} euphoriatheme          %b\n' "$(theme_status nebula)" "$(theme_status euphoriatheme)"
        printf '   ${WHITE}[3]${RESET} arix theme               %b    ${WHITE}[4]${RESET} stellar theme           %b\n' "$(theme_status arix)" "$(theme_status stellar)"
        printf '   ${WHITE}[5]${RESET} apollo theme             %b    ${WHITE}[6]${RESET} nexora theme             %b\n' "$(theme_status apollo)" "$(theme_status nexora)"
        printf '\n   ${RED}[0]${RESET}  Exit\n'
        printf '%b\n' " ${CYAN}──────────────────────────────────────────────────────────${RESET}"
        printf '%b' " ${WHITE}👉 Enter choice: ${RESET}"
        read -r choice
        case "$choice" in
            1|2|3|4|5|6)
                local name slug
                case "$choice" in
                    1) name='nebula theme'; slug='nebula' ;;
                    2) name='euphoriatheme'; slug='euphoriatheme' ;;
                    3) name='arix theme'; slug='arix' ;;
                    4) name='stellar theme'; slug='stellar' ;;
                    5) name='apollo theme'; slug='apollo' ;;
                    6) name='nexora theme'; slug='nexora' ;;
                esac
                loading "Opening ${name}"
                if theme_installed "$slug"; then
                    printf '%b\n' "${GREEN}${name} is already installed ●${RESET}"
                else
                    printf '%b\n' "${YELLOW}${name} is not installed ○${RESET}"
                    printf '%b\n' "${LIGHT_BLUE}Theme package/integration is required to install this theme.${RESET}"
                fi
                pause_menu ;;
            0) return ;;
