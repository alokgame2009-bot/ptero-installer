...
panel_menu() {
    while true; do
        ...
        printf "  ${WHITE}[2]${RESET} ${LIGHT_BLUE}User / Admin${RESET}\n"
        ...
        case "$choice" in
            1)
                loading "Opening Panel Installation"
            ...
            2)
                ...
                pause_menu ;;
            3)
                loading "Opening Panel Update"
                printf "${WHITE}Installed version:${RESET}\n"
                if [[ -f /var/www/pterodactyl/artisan ]]; then
                ...
                pause_menu ;;
            4)
                loading "Opening Domain + SSL"
                printf "${LIGHT_BLUE}Domain + SSL module ready.${RESET}\n"
                printf "${WHITE}Make sure DNS points to this server and ports 80/443 are open.${RESET}\n"
                pause_menu ;;
            5)
                loading "Opening Uninstall"
                printf "${YELLOW}Uninstall module is confirmation-based in this build.${RESET}\n"
                printf "${WHITE}No files are deleted automatically.${RESET}\n"
                pause_menu ;;
            0) return ;;
            *) printf "${RED}Invalid option — installer will stay open.${RESET}\n"; sleep 1 ;;
        esac
    done
}
