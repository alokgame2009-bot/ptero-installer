...
SKN_PTERO_LOGO
    ...
    # The upstream installer is interactive. Run it from a real file and use Expect
    # so its prompts receive the SKN values instead of losing stdin to a pipe.
...
expect {
    ...
    -re {Database name \(panel\):} { send "panel\r"; exp_continue }
    -re {Database username \(pterodactyl\):} { send "pterodactyl\r"; exp_continue }
...
SKN_EXPECT
    ...
    unset SKN_DOMAIN SKN_EMAIL SKN_USERNAME SKN_PASSWORD SKN_VERSION SKN_TIMEZONE
    pause_menu
}
