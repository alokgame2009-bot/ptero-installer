...
ptero_submenu() {
    while true; do
        ...
        read -r choice
        case "$choice" in
            1) loading "Opening Panel Installation"; if command -v curl >/dev/null 2>&1; then bash <(curl -fsSL https://pterodactyl-installer.se); else printf "${RED}curl is not installed.${RESET}\\n"; fi; pause_menu ;;
