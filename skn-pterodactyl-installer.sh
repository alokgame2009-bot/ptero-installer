...
SKN_PTERO_LOGO
    ...
    # Do not rewrite Ubuntu repositories or use Debian archive logic here.
    # This keeps Ubuntu 22.04/24.04 on their normal official mirrors.
...
SKN_APT_CONF
    ...
    # Finish an interrupted package transaction without attempting a potentially
    # blocking full dependency repair.
    ...
    # Ubuntu 22/24 normally provide universe, which some installer dependencies use.
    if command -v add-apt-repository >/dev/null 2>&1; then
        timeout 60 add-apt-repository -y universe >/tmp/skn-universe.log 2>&1 || true
    fi
    ...
    if ! timeout 180 env DEBIAN_FRONTEND=noninteractive apt-get update -o Acquire::Retries=3 >/tmp/skn-apt-update.log 2>&1; then
        printf '%b\n' "${RED}Ubuntu APT update failed.${RESET}"
        printf '%b\n' "${YELLOW}Last APT messages:${RESET}"
        tail -n 20 /tmp/skn-apt-update.log 2>/dev/null || true
        pause_menu; return
    fi

    # Install only the SKN automation dependencies. The upstream installer
    # handles PHP/MariaDB/Redis/Nginx and the rest of the Pterodactyl stack.
    printf '%b\n' "${LIGHT_BLUE}Installing SKN automation dependencies...${RESET}"
    if ! timeout 240 env DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends curl expect ca-certificates >/tmp/skn-deps-install.log 2>&1; then
    ...
    if ! command -v curl >/dev/null 2>&1 || ! command -v expect >/dev/null 2>&1; then
        printf '%b\n' "${RED}curl/expect are unavailable after package installation.${RESET}"
        pause_menu; return
    fi
