...
# Managed by SKN Pterodactyl Installer for Debian 11 Bullseye EOL compatibility.
...
SKN_APT_CONF
    ...
    # The upstream helper normally resolves the latest Panel release itself.
    # Replace only that release-resolution assignment so the selected SKN version
    # is the actual Panel release installed, not just a label in the UI.
    if grep -qE '^VERSION=.*get_latest_release.*pterodactyl/panel' "$installer"; then
        sed -i "s#^VERSION=.*#VERSION=\"${version}\"#" "$installer"
    elif grep -qE '^VERSION=.*pterodactyl/panel' "$installer"; then
        sed -i "s#^VERSION=.*#VERSION=\"${version}\"#" "$installer"
    else
        printf '%b\n' "${RED}Could not safely bind the selected Panel version to the upstream installer.${RESET}"
        rm -f "$installer"; pause_menu; return
    fi

    # Validate that the selected official Pterodactyl release exists before the
    # third-party installer is allowed to modify the system.
    if ! curl -fsI "https://github.com/pterodactyl/panel/releases/tag/${version}" >/dev/null; then
        printf '%b\n' "${RED}Selected Pterodactyl release ${version} could not be verified on GitHub.${RESET}"
        rm -f "$installer"; pause_menu; return
    fi
...
expect {
    ...
    # IMPORTANT: DNS mismatch is intentionally NOT auto-answered. If the
    # upstream installer asks this question, the installer stops and leaves
    # the decision to the user instead of silently forcing a broken HTTPS setup.
    -re {Proceed anyways \(your install will be broken if you do not know what you are doing\)\? \(y/N\):} {
        puts "\nSKN: DNS does not match the server IP. Fix the DNS record, then rerun the installer."
        send_user "SKN: Waiting for manual DNS decision (y/N): "
        expect_user -re "([yYnN])\r" { send "$expect_out(1,string)\r"; exp_continue }
    }
...
SKN_EXPECT
    ...
    if expect "$expect_script"; then
        if [[ -f /var/www/pterodactyl/artisan ]]; then
            ...
            printf '%b\n' "${LIGHT_BLUE}Requested Panel version: ${version}${RESET}"
    ...
    rm -f "$expect_script" "$installer"
    unset SKN_DOMAIN SKN_EMAIL SKN_USERNAME SKN_PASSWORD SKN_VERSION SKN_TIMEZONE SKN_DB_NAME SKN_DB_USER SKN_DB_PASSWORD
    pause_menu
}
